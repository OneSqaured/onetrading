...
PATH_DATABENTO = "data/CME_ES_TBBO_202402"

# Create an empty list to store the dataframes
result = []

# Use os.listdir to get the list of files in the directory
files = os.listdir(PATH_DATABENTO)
files.sort()
...
for filename in files:
    ...
    full_path = os.path.join(PATH_DATABENTO, filename)
...
conn = sqlite3.connect('data/onesquared.db')

# Optionally adjust SQLite settings for performance
conn.execute('PRAGMA journal_mode=WAL;')  # Switch to Write-Ahead Logging
conn.execute('PRAGMA cache_size = -10000;')  # Set cache size to 10000 pages (default page size is 4KB)
